...
import "./Homecss.css";
import { getNews, getProduct } from "./Axios";
import { slider, Pro_is_hot, whiteNews } from "./whiteData";
export default function Home(props) {
  ...
  const slideProduct = (item) => {
    return (
      <Carousel.Item>
        <Link
          onClick={() => {
            changeProductId(item._id);
          }}
          to={"/detail_product/" + item.name.replaceAll(" ", "_")}
        >
          <img
  ...
  const map_news = (item, index) => {
    ...
    return (
      <Link
        onClick={() => {
          changeNewsId(item._id);
        }}
        to={"/detail_news/" + item.title.replaceAll(" ", "_")}
      >
        <div className="news">
